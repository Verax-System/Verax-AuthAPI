...
from fastapi import APIRouter, Depends, HTTPException, status, Response, Path, BackgroundTasks
...
async def enable_mfa_confirm(
    *,
    ...
    return MFAConfirmResponse(
        user=UserSchema.model_validate(updated_user), # Usar model_validate (Pydantic v2)
...
async def verify_mfa_login(
    *,
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Token de desafio MFA inválido (sub inválido).")
...
async def refresh_access_token(*, db: AsyncSession = Depends(get_db), refresh_request: RefreshTokenRequest) -> Any:
    ...
    if not db_refresh_token or db_refresh_token.user_id != user_id:
        raise credentials_exception
    await crud_refresh_token.revoke_refresh_token(db, token=refresh_token_str) # Revogar o antigo
    user = await crud_user.get(db, id=user_id)
    if not user or not user.is_active:
        raise credentials_exception
    # Criar novos tokens
    new_access_token = security.create_access_token(user=user, mfa_passed=False) # mfa_passed=False no refresh
    new_refresh_token_str, new_expires_at = security.create_refresh_token(data={"sub": str(user.id)})
    new_expires_at_naive = new_expires_at.replace(tzinfo=None) if new_expires_at.tzinfo else new_expires_at
    await crud_refresh_token.create_refresh_token(db, user=user, token=new_refresh_token_str, expires_at=new_expires_at_naive) # Salvar o novo
...
async def verify_email(*, db: AsyncSession = Depends(get_db), token: str = Path(...)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Token de verificação inválido ou expirado")
...
async def forgot_password(*, db: AsyncSession = Depends(get_db), request_body: ForgotPasswordRequest, background_tasks: BackgroundTasks):
    ...
    if user and user.is_active:
        ...
        logger.warning(f"Tentativa de /forgot-password (email não existente ou inativo): {request_body.email}")
...
async def reset_password(*, db: AsyncSession = Depends(get_db), request_body: ResetPasswordRequest):
    ...
    if not payload or not payload.get("sub"):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Token de redefinição inválido ou expirado (JWT)")
    email = payload["sub"]
    user = await crud_user.get_user_by_reset_token(db, token=token)
    if not user or user.email != email:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Token de redefinição inválido, expirado ou já utilizado (DB)")
    try:
