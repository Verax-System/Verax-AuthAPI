...
# from app.api.dependencies import get_current_active_user, get_db # REMOVED
...
from app.schemas.user import User as UserSchema  # Importar UserSchema
...
# from app.api.dependencies import get_current_active_user, oauth2_scheme # REMOVED
...
# ... (Helpers MFA create/decode challenge token - sem alterações) ...
...
def create_mfa_challenge_token(user_id: int) -> str:
    ...
    encoded_jwt = jwt.encode(
        to_encode, MFA_CHALLENGE_SECRET_KEY, algorithm=MFA_CHALLENGE_ALGORITHM
    )
    return encoded_jwt
...
def decode_mfa_challenge_token(token: str) -> Dict | None:
    try:
        ...
        if payload.get("token_type") != "mfa_challenge":
            logger.warning(
                "Tentativa de usar token com tipo incorreto como challenge token MFA."
            )
            return None
        return payload
...
# ... (Endpoint /token - sem alterações relevantes para MyPy) ...
@router.post(
    "/token",
    response_model=Union[Token, MFARequiredResponse],
    responses={
        200: {
            "description": "Login bem-sucedido ou MFA necessário",
            "model": Union[Token, MFARequiredResponse],
        },
...
async def login_for_access_token(
    ...
    try:
        ...
        if e.locked_until:
            ...
            if e.locked_until > now:
                remaining_minutes = (
                    int((e.locked_until - now).total_seconds() // 60) + 1
                )
                detail_msg = (
    ...
    if user.is_mfa_enabled:
        ...
        logger.info(
            f"Login para {user.email}: MFA necessário, challenge token emitido."
        )
    ...
    requested_scopes_list = form_data.scopes if form_data.scopes else []
    access_token = security.create_access_token(
        user=user, requested_scopes=requested_scopes_list, mfa_passed=False
    )
    refresh_token_str, expires_at = security.create_refresh_token(
        data={"sub": str(user.id)}
    )
    # Certifique-se de que expires_at seja timezone-naive para o banco de dados
    expires_at_naive = (
        expires_at.replace(tzinfo=None) if expires_at.tzinfo else expires_at
    )
    await crud_refresh_token.create_refresh_token(
        db, user=user, token=refresh_token_str, expires_at=expires_at_naive
    )
    response.status_code = status.HTTP_200_OK
    return Token(
...
# ... (Endpoints Google OAuth - sem alterações relevantes para MyPy) ...
...
async def get_google_login_url():
    if not settings.GOOGLE_CLIENT_ID or not settings.GOOGLE_REDIRECT_URI_FRONTEND:
        logger.error(
            "GOOGLE_CLIENT_ID ou GOOGLE_REDIRECT_URI_FRONTEND não estão configurados no .env"
        )
        raise HTTPException(
            status_code=500, detail="Configuração OAuth está incompleta."
        )
...
async def google_callback(
    *, db: AsyncSession = Depends(get_db), login_request: GoogleLoginRequest
):
    code = login_request.code
    if (
        not settings.GOOGLE_CLIENT_ID
        or not settings.GOOGLE_CLIENT_SECRET
        or not settings.GOOGLE_REDIRECT_URI_FRONTEND
    ):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Código de autorização inválido ou expirado."
            )
        except Exception as e:
            logger.error(f"Erro de rede ao contactar Google Token URL: {e}")
            raise HTTPException(
                status_code=500, detail="Erro ao contactar serviço de login."
            )
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Falha ao obter dados do utilizador."
            )
    ...
    if not user_info.get("email_verified"):
        raise HTTPException(
            status_code=400, detail="Email da Google não está verificado."
        )
...
# ... (Endpoint /mfa/enable - sem alterações relevantes para MyPy) ...
...
async def enable_mfa_start(
    ...
    try:
        await crud_user.set_pending_otp_secret(
            db=db, user=current_user, otp_secret=otp_secret
        )
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.error(
            f"Erro ao salvar segredo OTP pendente para {current_user.email}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="Erro ao iniciar habilitação do MFA."
        )
    ...
    try:
        ...
        qr_code_base64 = ""
    logger.info(
        f"Iniciada habilitação MFA para {current_user.email}. Segredo pendente salvo."
    )
...
async def enable_mfa_confirm(
    *,
    ...
    result = await crud_user.confirm_mfa_enable(
        db=db, user=current_user, otp_code=mfa_data.otp_code
    )

    if not result:
        raise HTTPException(
            status_code=400, detail="Código OTP inválido ou falha ao confirmar MFA."
        )

    updated_user, plain_recovery_codes = result

    # CORRIGIDO: Converter o modelo SQLAlchemy para o schema Pydantic
...
# ... (Restante dos endpoints MFA e outros - sem alterações relevantes para MyPy ou já corrigidos antes) ...
...
async def disable_mfa(
    *,
    ...
    # FastAPI/Pydantic handle conversion from ORM model here if response_model is set
    return updated_user


@router.post("/mfa/verify", response_model=Token)
async def verify_mfa_login(
    *, db: AsyncSession = Depends(get_db), mfa_data: MFAVerifyRequest
):
    payload = decode_mfa_challenge_token(mfa_data.mfa_challenge_token)
    if not payload:
        raise HTTPException(
            status_code=400, detail="Token de desafio MFA inválido ou expirado."
        )

    user_id_str = payload.get("sub")
    if not user_id_str:
        raise HTTPException(
            status_code=400, detail="Token de desafio MFA inválido (sem sub)."
        )
    try:
        user_id = int(user_id_str)
    except ValueError:
        raise HTTPException(
            status_code=400, detail="Token de desafio MFA inválido (sub inválido)."
        )
    ...
    if not user or not user.is_active or not user.is_mfa_enabled or not user.otp_secret:
        ...
        raise HTTPException(
            status_code=400,
            detail="Usuário inválido ou MFA não está (mais) habilitado.",
        )
    ...
    logger.info(
        f"Verificação MFA (OTP) bem-sucedida para {user.email}. Emitindo tokens."
    )
    access_token = security.create_access_token(user=user, mfa_passed=True)
    refresh_token_str, expires_at = security.create_refresh_token(
        data={"sub": str(user.id)}
...
async def verify_mfa_recovery_login(
    *, db: AsyncSession = Depends(get_db), mfa_data: MFARecoveryRequest
):
    payload = decode_mfa_challenge_token(mfa_data.mfa_challenge_token)
    if not payload:
        raise HTTPException(
    ...
    if not user or not user.is_active or not user.is_mfa_enabled:
        ...
        raise HTTPException(
            status_code=400, detail="Usuário inválido ou MFA não está habilitado."
        )

    db_code = await crud_mfa_recovery_code.get_valid_recovery_code(
        db=db, user=user, plain_code=mfa_data.recovery_code
    )

    if not db_code:
        logger.warning(
            f"Código de recuperação inválido ou já utilizado para {user.email}."
        )
        raise HTTPException(
            status_code=400, detail="Código de recuperação inválido ou já utilizado."
        )

    await crud_mfa_recovery_code.mark_code_as_used(db=db, db_code=db_code)

    logger.info(
        f"Verificação MFA (RECOVERY CODE) bem-sucedida para {user.email}. Emitindo tokens."
    )
    access_token = security.create_access_token(user=user, mfa_passed=True)
    refresh_token_str, expires_at = security.create_refresh_token(
        data={"sub": str(user.id)}
...
async def refresh_access_token(
    *, db: AsyncSession = Depends(get_db), refresh_request: RefreshTokenRequest
) -> Any:
    ...
    try:
        ...
        raise credentials_exception
    db_refresh_token = await crud_refresh_token.get_refresh_token(
    ...
    new_access_token = security.create_access_token(
        user=user, mfa_passed=False
    )  # mfa_passed=False no refresh
    new_refresh_token_str, new_expires_at = security.create_refresh_token(
        data={"sub": str(user.id)}
    )
    new_expires_at_naive = (
        new_expires_at.replace(tzinfo=None) if new_expires_at.tzinfo else new_expires_at
    )
    await crud_refresh_token.create_refresh_token(
        db, user=user, token=new_refresh_token_str, expires_at=new_expires_at_naive
    )  # Salvar o novo
    return Token(
...
async def logout(
    *, db: AsyncSession = Depends(get_db), refresh_request: RefreshTokenRequest
    ...
    await crud_refresh_token.revoke_refresh_token(
        db, token=refresh_request.refresh_token
    )
...
async def read_users_me(
    ...
    # FastAPI/Pydantic handle conversion from ORM model here
    return current_user
...
async def forgot_password(
    *,
    ...
    if user and user.is_active:
        try:
            db_user, reset_token = await crud_user.generate_password_reset_token(
                db, user=user
            )
            ...
            logger.error(
                f"Erro no fluxo /forgot-password para {request_body.email}: {e}"
            )
    else:
        # Não vazar informação se o email existe ou não, logar apenas
        logger.warning(
            f"Tentativa de /forgot-password (email não existente ou inativo): {request_body.email}"
        )
    # Sempre retornar a mesma mensagem genérica por segurança
    return {
        "msg": "Se um usuário com esse email existir e estiver ativo, um link de redefinição será enviado."
    }


@router.post("/reset-password", response_model=UserSchema)
async def reset_password(
    *, db: AsyncSession = Depends(get_db), request_body: ResetPasswordRequest
):
    ...
    try:
        ...
        return updated_user  # FastAPI/Pydantic handle conversion
    except Exception as e:
        logger.error(f"Erro ao tentar redefinir a senha para {user.email}: {e}")
        await db.rollback()  # Garantir rollback em caso de erro no commit
        raise HTTPException(
